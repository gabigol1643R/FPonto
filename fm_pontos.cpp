...
fm_pontos::fm_pontos(QWidget *parent) : QDialog(parent), ui(new Ui::fm_pontos) {
  ui->setupUi(this);
  ...
  QSqlQuery query;
  query.prepare(
      "select p.id_colab,c.nome_colab,p.data,p.entrada,p.saida,p.total "
      "from ponto as p inner join colaborador as c on p.id_colab=c.id_colab");
  if (query.exec()) {
    int linha = 0;
    while (query.next()) {
      ui->tb_pontos->insertRow(linha);
      ui->tb_pontos->setItem(linha, 0,
                             new QTableWidgetItem(query.value(0).toString()));
      ui->tb_pontos->setItem(linha, 1,
                             new QTableWidgetItem(query.value(1).toString()));
      ui->tb_pontos->setItem(linha, 2,
                             new QTableWidgetItem(query.value(2).toString()));
      ui->tb_pontos->setItem(linha, 3,
                             new QTableWidgetItem(query.value(3).toString()));
      ui->tb_pontos->setItem(linha, 4,
                             new QTableWidgetItem(query.value(4).toString()));
      ui->tb_pontos->setItem(linha, 5,
